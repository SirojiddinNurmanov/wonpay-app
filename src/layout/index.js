...
const Layout = ({ children, buttons, title = false, empty = false }) => {
    ...
    if (user?.user.role === "client") {
        ...
        balanceText = "$" + formatAmount(user?.user.balance + (carriers?.length > 0 ? carriers?.map(carrier => carrier.balance).reduce((sum, carrierBalance) => sum + carrierBalance) : 0) || 0, true, true) + ` ( $${formatAmount(user?.user.balance, true)} )`;
