...
const ProcessesPage = () => {
    ...
    return (
        <Layout buttons={common}>
            <Link to="/queries">
                <Title text="So'rovlar:" amount={"₩" + (queries ? formatAmount(sumProcessAmount(queries.filter(query => query.assigned_offer === null).filter(query => query.status !== 1))) : "0")} inContent={true} />
            </Link>
